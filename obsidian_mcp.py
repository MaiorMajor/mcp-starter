...
_vault_env = os.getenv("VAULT_PATH", "").strip()
if not _vault_env:
    raise RuntimeError("VAULT_PATH must be set in .env or the environment.")
VAULT_PATH: Path = Path(_vault_env)
...
@lru_cache(maxsize=1)
def _vault_file_search_module():
    vfs_path = REPO_ROOT / "vault_file_search.py"
    if not vfs_path.exists():
        raise FileNotFoundError(f"vault_file_search module not found: {vfs_path}")
    spec = importlib.util.spec_from_file_location("vault_file_search", vfs_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load vault_file_search from {vfs_path}")
...
@lru_cache(maxsize=1)
def _vault_json_reader_module():
    vjr_path = REPO_ROOT / "vault_json_reader.py"
    if not vjr_path.exists():
        raise FileNotFoundError(f"vault_json_reader module not found: {vjr_path}")
    spec = importlib.util.spec_from_file_location("vault_json_reader", vjr_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"cannot load vault_json_reader from {vjr_path}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = mod
    spec.loader.exec_module(mod)
    return mod
...
def _read_json(args: dict) -> dict:
    ...
    try:
        mod = _vault_json_reader_module()
        fields = args.get("fields")
        if fields is not None and not isinstance(fields, list):
            fields = [str(fields)]
        result = mod.read_json(
            file_path,
...
def _find_files(args: dict) -> dict:
    try:
        vfs = _vault_file_search_module()
    except (FileNotFoundError, ImportError) as exc:
        return {
            "error": str(exc),
            "files": [],
            "count": 0,
            "hint": "Fallback: run_skill('vault-find', ['--ext', '.pdf', ...])",
        }
...
_SYSTEM_PROMPT_PATH = REPO_ROOT / "system_prompt.md"
_FALLBACK_PROMPT = (
